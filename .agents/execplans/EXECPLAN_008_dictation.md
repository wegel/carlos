...
- Observation: whisper.cpp emits model-load and decoder diagnostics directly to stderr unless the `whisper-rs` logging hooks are installed.
  Evidence: the user saw `whisper_init_state` and related native messages polluting the TUI during otherwise correct transcription. Running the ignored model-backed integration test with `--nocapture` reproduced the same diagnostics before the fix. After moving `install_logging_hooks()` before `WhisperContext::new_with_params`, the same command produced only the Rust test harness output and no native Whisper diagnostics.
...
- Decision: Install the `whisper-rs` logging hooks immediately before loading each Whisper model.
  Rationale: The hooks are process-global and safe to call more than once. Installing at model load covers both the real worker path and model-backed tests that call `load_model()` directly, and because this crate does not enable `whisper-rs` log or tracing backends the native messages are suppressed instead of being sent to the terminal.
  Date/Author: 2026-04-29 / codex
...
2026-04-29 / codex: Fixed the TUI pollution reported during live dictation by installing `whisper-rs` logging hooks before model load. The model-backed integration smoke now runs without native Whisper logs under `--nocapture`, and the updated dictation-enabled release binary is installed.
