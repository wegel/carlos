...
- [x] (2026-04-29 20:25Z) Human instruction received: do not run a re-review. At that point completion was blocked on the Milestone 5 manual French-profile, English-profile, and missing-model smokes because the environment had no `~/.config/carlos/dictation.toml` and no `~/.cache/carlos` model directory. That blocker was resolved by the later local model/profile setup and live confirmation entries.
...
- [x] (2026-04-29 20:38Z) Remaining Milestone 5 acceptance required live manual microphone smokes for the configured French and English profiles, plus the missing-model TUI smoke. The user later tested the live profiles, reported that both transcriptions worked correctly, reported a log-pollution regression, then confirmed that the final installed fix works.
- [x] (2026-04-29 20:48Z) Suppressed Whisper/GGML native logging that polluted the TUI during model load and transcription. The first attempt used `whisper_rs::install_logging_hooks()`, but the user reported the live profiles then returned only `"."`. That hook-based approach was replaced with narrow stderr redirection around native model load, state creation, and inference, preserving Whisper's default callback behavior while keeping native diagnostics out of the terminal. Evidence: the ignored raw-f32 integration test against `ggml-large-v3-turbo-q5_0.bin` now passes under `--nocapture`, prints the expected JFK transcript containing `country`, and emits no native `whisper_init_state`/`whisper_full_with_state` output. Validation: `cargo test --features dictation` passed with 293 tests and 1 ignored integration test, `cargo test` passed with 257 tests, `cargo build --no-default-features` passed, `cargo build --release --features dictation` passed, and `install -Dm755 target/release/carlos ~/.local/bin/carlos` installed the updated dictation-enabled binary.
- [x] (2026-04-29 21:02Z) Final closeout: the user confirmed the installed dictation-enabled binary works after the scoped stderr suppression fix. This ExecPlan is complete and ready to move unchanged to `.agents/done/`.
...
- Observation: whisper.cpp emits model-load, state-creation, and decoder diagnostics directly to stderr.
  Evidence: the user saw `whisper_init_state` and related native messages polluting the TUI during otherwise correct transcription. Running the ignored model-backed integration test with `--nocapture` reproduced the same diagnostics before the fix. A first hook-based suppression removed the diagnostics but was followed by live `"."` transcriptions, so the current fix redirects stderr to `/dev/null` only while calling `WhisperContext::new_with_params`, `create_state`, and `full`. The same fixture test now prints `dictation fixture transcript: "And so, my fellow Americans, ask not what your country can do for you, ask what you can do for your country."` without native diagnostics.

- Observation: The final installed dictation build works in live user testing.
  Evidence: after installing the scoped-stderr release build to `~/.local/bin/carlos`, the user retried live dictation and reported "yes it works now".
...
- Decision: Suppress Whisper native stderr output with scoped file-descriptor redirection instead of `whisper-rs` logging hooks.
  Rationale: The hook-based approach was clean for display output but the user reported live `"."` transcriptions immediately after it was installed. Scoped stderr redirection keeps Whisper's internal logging callback configuration unchanged, limits suppression to the known noisy native calls, and still prevents terminal corruption in the TUI.
  Date/Author: 2026-04-29 / codex
...
2026-04-29 / codex: Fixed the TUI pollution reported during live dictation without changing Whisper's logging callbacks by redirecting stderr around native model load/state/inference calls. The model-backed integration smoke now produces the expected JFK transcript without native Whisper logs under `--nocapture`, and the updated dictation-enabled release binary is installed. This supersedes the earlier hook-based attempt, which the user reported caused live transcriptions to become only `"."`.

2026-04-29 / codex: Completed the ExecPlan. Carlos now has optional in-process dictation behind the `dictation` Cargo feature, local `fr-qc` and `en` dictation profiles, downloaded large-v3-turbo GGML models, bounded microphone capture and worker handoff, runtime `F7` profile cycling, cancellation/stale-event protection, README guidance, and an installed dictation-enabled release binary. Automated validation passed after the final fix, and the user confirmed live dictation works. Remaining context: `stt.md` is an untracked source note that was intentionally not committed.
